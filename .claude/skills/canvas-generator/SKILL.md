...
description: [画布生成] | 基于文本自动创建及布局 Obsidian .canvas 文件 | 直观的知识思维链路图
---
...
**输出路径规则：**
- Canvas 文件必须与源笔记放在同一目录
- 文件命名：`<笔记名>-架构图.canvas` 或 `<主题>-思维导图.canvas`
- **严禁扫描全库**，仅读取用户指定的目录或文件

**路径示例：**
```
源笔记: 20-知识库/Java/虚拟线程.md
Canvas: 20-知识库/Java/虚拟线程-架构图.canvas

源笔记: 30-资源/计算机网络/TCP拥塞控制.md
Canvas: 30-资源/计算机网络/TCP拥塞控制-思维导图.canvas
```
