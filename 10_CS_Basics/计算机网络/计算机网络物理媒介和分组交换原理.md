...
public class NetworkDelayDemo {
    ...
    static double storeAndForwardDelay(int hops, double bits, double bandwidthBps,
        ...
        // 为什么：逐跳累加更贴合存储转发；底层：每跳先收满分组再查表转发，时延可叠加
    ...
    public static void main(String[] args) {
        ...
        double bandwidth = 100_000_000; // 为什么：用链路速率；底层：决定发送时延上限
        double propPerHop = propagationDelay(100_000, 2.0e8); // 为什么：用物理距离与介质速度；底层：决定传播时延
        double procPerHop = 0.0002; // 为什么：给路由器处理时间；底层：查表/校验的处理开销
        double queuePerHop = 0.001; // 为什么：给排队等待时间；底层：缓冲区排队导致的时延
